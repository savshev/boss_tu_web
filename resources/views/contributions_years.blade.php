    ...
    <div class="years-list" id="yearsList" tabindex="0">
        @forelse($yearsData as $index =>$row)
            @php
                ...
                $countVal = (int) ($arr['COUNT'] ?? $arr['count'] ?? 0);$summaAvg = (float) ($arr['SUMMA'] ?? $arr['summa'] ?? 0);
                $summaYear =$summaAvg * 12; // Множимо середній внесок на 12
                $fmtSumma = number_format($summaYear, 2, '.', '');
            @endphp
            <div class="year-item"
                 data-year="{{ $yearVal }}"
                 data-count="{{ $countVal }}"
                 data-summa="{{ $summaYear }}"
                 data-index="{{ $index }}"
                 tabindex="0">
...
<div class="modal-overlay" id="chartModal">
    <div class="modal-content">
        <div class="modal-header">
            <span>Графік чисельності та внесків по роках</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let savedIndex = sessionStorage.getItem('active_vzcn_year_index');
            ...
            options: {
                ...
                scales: {
                    ...
                    ySumma: {
                        ...
                        display: true,
                        position: 'left',
                        title: { display: true, text: 'Внески (грн)' }
                    },
                    yCount: {
                        type: 'linear',
                        display: true,
                        ...
                        title: { display: true, text: 'Чисельність (осіб)' }
                    }
                }
            }
        });
