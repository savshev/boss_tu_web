    ...
    <div class="dprt-list" id="dprtList" tabindex="0">
        @forelse($departments as $index =>$dprt)
            @php
                ...
                // Безпечне зчитання значень незалежно від регістру
                $id       = trim((string)($arr['DEPARTMN'] ?? $arr['departmn'] ?? ''));
                $name     = trim((string)($arr['DPRT_INFO'] ?? $arr['dprt_info'] ?? ''));$countAll = (int) ($arr['COUNT'] ?? $arr['count'] ?? 0);
                $countMen = (int) ($arr['COUNT_MEN'] ?? $arr['count_men'] ?? 0);$countWom = (int) ($arr['COUNT_WOM'] ?? $arr['count_wom'] ?? 0);
            @endphp
            <div class="dprt-item"
                 data-id="{{ $id }}"
                 data-index="{{ $index }}"
